    ...
    try {
        ...
        if (role === 'admin') {
            ...
            try {
                ...
                const userUrl = `index.html?id=${newId}`;
        ...
        const userData = {
            ...
            // อ่านลายเซ็นที่ user แนบ (จาก preview) เป็น dataURL
            const userImg1 = document.querySelector('#userPreview1 img');
            const userImg2 = document.querySelector('#userPreview2 img');
            const userName = document.querySelector('.signature-line.user-input')?.value
                || params.get('name') || localStorage.getItem('name') || '';

            userData.name = userName;
            userData.signatures = {};
            if (userImg1 && userImg1.src) userData.signatures['sig1'] = userImg1.src;
            if (userImg2 && userImg2.src) userData.signatures['sig2'] = userImg2.src;

            await update(ref(db, `checklists/${assetId}/userData`), userData);

            // 👉 ลิงก์กลับไปให้ Admin
            const adminUrl = `index.html?role=admin&id=${encodeURIComponent(assetId)}`;
